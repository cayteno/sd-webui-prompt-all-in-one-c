...
from scripts.physton_prompt.manage_group_tags import (
    get_data as manage_get_data,
    save_data as manage_save_data,
    list_files as manage_list_files,
    translate_free as manage_translate_free,
    download_file as manage_download_file,
    import_yaml_text as manage_import_yaml,
)
...
def on_app_started(_: gr.Blocks, app: FastAPI):
    ...
    async def _get_group_tags(lang: str):
        remote_url = None
        try:
            remote_url = st.get('remote_yaml_url')
            if not remote_url:
                remote_url = None
        except Exception:
            remote_url = None
        return {"tags": get_group_tags(lang, remote_url)}

    @app.get("/physton_prompt/manage/group_tags")
    async def _manage_group_tags_get(file: str = 'prepend.yaml'):
        return manage_get_data(file)

    @app.get("/physton_prompt/manage/group_tags/files")
    async def _manage_group_tags_files():
        return {"files": manage_list_files()}

    @app.post("/physton_prompt/manage/translate")
    async def _manage_translate(request: Request):
        try:
            data = await request.json()
        except Exception:
            data = {}
        text = data.get('text') or ''
        return manage_translate_free(text)

    @app.post("/physton_prompt/manage/group_tags/save")
    async def _manage_group_tags_save(request: Request):
        try:
            data = await request.json()
            return manage_save_data(data.get('data'), data.get('file', 'prepend.yaml'))
        except Exception as e:
            return {"success": False, "message": str(e)}

    @app.get("/physton_prompt/manage/group_tags/download")
    async def _manage_group_tags_download(file: str = 'prepend.yaml'):
        return manage_download_file(file)

    # 远程 YAML 文件 API（仅开放 prepend.yaml / default.yaml / zh_CN.yaml）
    ALLOWED_REMOTE_FILES = {'prepend.yaml', 'default.yaml', 'zh_CN.yaml'}

    @app.get("/physton_prompt/remote_yaml")
    async def _remote_yaml(file: str):
        if file not in ALLOWED_REMOTE_FILES:
            return Response(content='{"success":false,"message":"file not allowed"}', status_code=403, media_type='application/json')
        file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../group_tags/', file)
        file_path = os.path.normpath(file_path)
        if not os.path.exists(file_path):
            return Response(content='{"success":false,"message":"file not found"}', status_code=404, media_type='application/json')
        return FileResponse(file_path, media_type='text/yaml; charset=utf-8', filename=file)
    @app.get("/physton_prompt/proxy_yaml")
    async def _proxy_yaml(url: str):
        """Server-side proxy to fetch remote YAML, bypassing browser CORS."""
        ...
        try:
            ...
            return {"success": False, "message": f"URL Error: {str(e.reason)}"}
        except Exception as e:
            return {"success": False, "message": str(e)}


    @app.post("/physton_prompt/manage/group_tags/import")
    async def _manage_group_tags_import(request: Request):
        try:
            form = await request.form()
            file_name = form.get('file', 'prepend.yaml')
            upload = form.get('upload')
            if not upload:
                return {"success": False, "message": "未选择文件"}
            content = await upload.read()
            text = content.decode('utf-8-sig')
            return manage_import_yaml(text, file_name)
        except Exception as e:
            return {"success": False, "message": str(e)}
